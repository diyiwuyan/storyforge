...
export class DeepSeekProvider implements LLMProvider {
  ...
  async chat(messages: ChatMessage[], options?: ChatOptions): Promise<ChatResponse> {
    ...
    // 120-second timeout to handle slow responses gracefully
    ...
    try {
      ...
      if (fetchErr instanceof Error && fetchErr.name === 'AbortError') {
        throw new Error('[DeepSeek] API 请求超时（120 秒），请检查网络或更换服务商');
      }
      throw new Error(`[DeepSeek] 网络请求失败: ${fetchErr instanceof Error ? fetchErr.message : String(fetchErr)}`);
    } finally {
      clearTimeout(timeoutId);
    }
